...
Riposte uses URI Template as its language for URIs. URI Template extends URIs (aka URLs) with a kind of template syntax. Values get plugged in between open and close curly braces. Here, we plug in the value of the @tt{uuid} variable, whose value came from the environment. The @tt{2XX} means: we expect a 200-class response; the precise response code doesn't matter. It could be 200, 201, 204…
...
Here, we define some data and build a JSON object with three properties. We then submit that to (what works out to) @tt{https://api.vicampo.de/v1/checkout/cart/28f896e6-98df-497b-b95f-c34d39c2a368/items}. (I just picked a random UUID.) Notice that we expect a concrete response code here. The response had better be 200. Anything else would be an error.
